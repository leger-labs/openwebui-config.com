...
if (!fs.existsSync(OPENAPI_SPEC_PATH)) {
  ...
  console.error('   Please ensure /schemas/openwebui-config-schema.json exists');
  process.exit(1);
}
...
const configSchema = convertToJsonSchema(configDefinition);
const uiSchema = generateUiSchema(configDefinition);
